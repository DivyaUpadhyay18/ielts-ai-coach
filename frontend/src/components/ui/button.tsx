...
export interface ButtonProps
  ...
  /** When true, renders the single child element (e.g. a <Link>) with the
   *  button's styles and other props merged onto it (shadcn Slot pattern). */
  asChild?: boolean;
}

/** Minimal Slot: merge props/className onto the single child element. */
function Slot({
  children,
  className,
  ...props
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "default", size = "default", isLoading, leftIcon: LeftIcon, rightIcon: RightIcon, children, disabled, asChild = false, ...props }, ref) => {
    
    // Base styles + variants mapping
    const variants = {

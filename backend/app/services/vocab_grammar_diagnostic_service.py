"""
Vocabulary & Grammar Diagnostic Module service.

Deterministic (NO AI) assessment of IELTS Vocabulary and Grammar using eight
question types:
...
Responsibilities:
  - fetch the vocabulary/grammar question bank
  ...
  - compute and STORE per-attempt results:
      ...
      * weak grammar topics (grammar types below threshold)
      * weak vocabulary categories (vocabulary types below threshold)
      * time (total + per question-type average)
      * difficulty level (Easy / Moderate / Hard)
      * estimated IELTS band
  - generate a report and persist it in `vocab_grammar_diagnostic_results`

The module reuses `diagnostic_attempts` for the lifecycle/resume semantics and
records per-question responses in `diagnostic_responses` to keep one source of
truth for progress.
"""
import logging
import re
...
from app.models.vocab_grammar_diagnostic import (
    GRAMMAR_QUESTION_TYPES,
    VOCABULARY_QUESTION_TYPES,
)
from app.repositories.diagnostic_repo import DiagnosticRepository
from app.repositories.vocab_grammar_diagnostic_repo import VocabGrammarDiagnosticRepository
...
# Band floor/ceiling derived from accuracy (%).
BAND_FLOOR = 3.0
BAND_CEIL = 9.0

# A question type is "weak" when its accuracy is below this %.
WEAK_TYPE_THRESHOLD = 60.0

# A question type is "strong" when its accuracy is at/above this %.
STRONG_TYPE_THRESHOLD = 80.0

# Difficulty level thresholds based on average question difficulty (1-5).
...
class VocabGrammarDiagnosticService:
    """Business logic for the Vocabulary & Grammar Diagnostic Module."""

    def __init__(self, db: DatabaseSession) -> None:
        self.db = db
        self.repo = VocabGrammarDiagnosticRepository(db)
    ...
    def get_bank(self) -> Dict[str, Any]:
        """
        Return all active vocabulary/grammar questions.

        The correct answer is stripped so the client cannot see it in advance.
        """
        questions = self.repo.get_questions()
        bank = []
        for q in questions:
            qtype = q.get("question_type") or "fill_in_the_blanks"
            section = q.get("section")
            if section not in ("vocabulary", "grammar"):
                continue
            bank.append({
                "id": q["id"],
                "section": section,
                ...
                "time_limit_seconds": int(q.get("time_limit_seconds") or 45),
                "skill_tag": q.get("skill_tag"),
            })
        return {"questions": bank, "total": len(bank)}
    ...
    def submit_answer(
        ...
        """Grade and persist a single vocabulary/grammar answer."""
        ...
        if not question:
            raise NotFoundError("Vocabulary/grammar question not found")

        correct = question.get("answer")
        is_correct = self._check_answer(answer, correct)
        time_taken = min(int(time_taken_seconds or 0), int(question.get("time_limit_seconds") or 45))

        section = question.get("section")
        qtype = question.get("question_type") or "fill_in_the_blanks"
        ...
        self.diag_repo.save_response(
            ...
            attempt_id,
            {
                ...
                "answer_json": {"value": answer, "question_type": qtype},
    ...
    def complete_attempt(
        ...
        """Finalize a vocabulary/grammar attempt, compute metrics, and store results."""
        attempt = self.diag_repo.get_attempt(attempt_id, user_id)
        ...
        responses = [
            ...
            if r.get("section") in ("vocabulary", "grammar")
        ]

        # Only count responses that map to known vocabulary/grammar questions.
        graded = [
        ...
        # Grammar vs vocabulary accuracy.
        grammar_accuracy = self._section_accuracy(graded, question_map, "grammar")
        vocabulary_accuracy = self._section_accuracy(graded, question_map, "vocabulary")

        # Weak grammar topics (grammar types below threshold).
        weak_grammar_topics = sorted(
            [t for t, b in type_breakdown.items() if b["section"] == "grammar" and b["accuracy"] < WEAK_TYPE_THRESHOLD],
            key=lambda t: type_accuracy[t],
        )
        # Weak vocabulary categories (vocabulary types below threshold).
        weak_vocab_categories = sorted(
            [t for t, b in type_breakdown.items() if b["section"] == "vocabulary" and b["accuracy"] < WEAK_TYPE_THRESHOLD],
            key=lambda t: type_accuracy[t],
        )
        strong_types = sorted(
        ...
        # Persist the vocabulary/grammar result.
        self.repo.save_result(user_id, {
            ...
            "grammar_accuracy": round(grammar_accuracy, 1),
            "vocabulary_accuracy": round(vocabulary_accuracy, 1),
            "type_accuracy": type_accuracy,
            "type_time": type_time,
            "weak_grammar_topics": weak_grammar_topics,
            "weak_vocab_categories": weak_vocab_categories,
            "strong_types": strong_types,
            "detail": detail,
            "completed_at": datetime.utcnow().isoformat(),
        })

        # Mark vocabulary & grammar as completed on the shared attempt.
        ...
        logger.info(
            "vocab/grammar diagnostic done user=%s attempt=%s acc=%.1f band=%.1f",
            user_id, attempt_id, accuracy, band,
        )
        return self.build_report(user_id, attempt_id)

    def build_report(self, user_id: str, attempt_id: str) -> Dict[str, Any]:
        """Build (and return) the vocabulary/grammar diagnostic report."""
        stored = self.repo.get_result(attempt_id, user_id)
        if not stored:
            raise NotFoundError("Vocabulary/grammar result not found")
        ...
        return {
            ...
            "grammar_accuracy": float(stored.get("grammar_accuracy") or 0.0),
            "vocabulary_accuracy": float(stored.get("vocabulary_accuracy") or 0.0),
            "total_time_seconds": int(stored.get("total_time_seconds") or 0),
            "band": float(stored.get("band") or 0.0),
            "difficulty_level": stored.get("difficulty_level") or "Easy",
            "type_breakdown": breakdown,
            "weak_grammar_topics": stored.get("weak_grammar_topics") or [],
            "weak_vocab_categories": stored.get("weak_vocab_categories") or [],
            "strong_types": stored.get("strong_types") or [],
            "completed_at": completed_at,
        }

    def list_results(self, user_id: str, limit: int = 20) -> Dict[str, Any]:
        """Return a user's stored vocabulary/grammar diagnostic results."""
        ...
        for r in rows:
            results.append({
                ...
                "grammar_accuracy": float(r.get("grammar_accuracy") or 0.0),
                "vocabulary_accuracy": float(r.get("vocabulary_accuracy") or 0.0),
                "band": float(r.get("band") or 0.0),
                "difficulty_level": r.get("difficulty_level"),
                "weak_grammar_topics": r.get("weak_grammar_topics") or [],
                "weak_vocab_categories": r.get("weak_vocab_categories") or [],
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _check_answer(answer: Any, correct: Any) -> bool:
        """Compare a user answer to the correct answer (case-insensitive, normalized)."""
        if correct is None:
            return False
    ...
    def _compute_type_breakdown(
        ...
        for r in responses:
            ...
            qtype = (q or {}).get("question_type") or "fill_in_the_blanks"
            section = (q or {}).get("section") or "vocabulary"
            entry = by_type.setdefault(qtype, {"total": 0, "correct": 0, "time": 0.0, "section": section})
            entry["total"] += 1
            if r.get("is_correct"):
                entry["correct"] += 1
    ...
    @staticmethod
    def _section_accuracy(
        responses: List[Dict[str, Any]],
        question_map: Dict[str, Dict[str, Any]],
        section: str,
    ) -> float:
        """Accuracy (%) for a specific section (vocabulary or grammar)."""
        section_responses = [
            r for r in responses
            if (question_map.get(r.get("question_id") or {}) or {}).get("section") == section
        ]
        if not section_responses:
            return 0.0
        correct = sum(1 for r in section_responses if r.get("is_correct"))
        return (correct / len(section_responses)) * 100.0
    ...
    def _accuracy_to_band(self, accuracy: float) -> float:
        """Map accuracy % to IELTS band (0.5 steps)."""
    ...
    def _report_breakdown(stored: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for qtype, acc in type_accuracy.items():
            section = "grammar" if qtype in GRAMMAR_QUESTION_TYPES else "vocabulary"
            breakdown.append({
                "question_type": qtype,
                "section": section,
                "total": 0,
...
vocab_grammar_diagnostic_service = VocabGrammarDiagnosticService(db_session)
